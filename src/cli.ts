...
program.name("codex-butler").description("A productivity and diagnostics layer for OpenAI Codex").version("0.7.0");
...
program.command("init").description("Analyze the current project and create AGENTS.md plus project memory").option("--force", "overwrite an existing AGENTS.md").action(async (options: { force?: boolean }) => {
  ...
  if (project.truncated) console.log(pc.yellow("! Project scan was truncated (file or depth limit). AGENTS.md still covers the sampled tree."));
});
